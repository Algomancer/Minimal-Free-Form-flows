...
formula is estimated via a Hutchinson surrogate built from forward-mode
...
#
#   c = J_dec(z) v            forward-mode JVP through the decoder, stop-gradient
#   S = v . (J_enc(x) c)      forward-mode JVP through the encoder
#
# Both Jacobian-vector products are forward-mode, so grad_theta S is a single
# backward pass.
#
...
def hutchinson_surrogate(
    ...
    _, jvp_enc = fwd_jvp(encoder, x, jvp_dec.detach())
